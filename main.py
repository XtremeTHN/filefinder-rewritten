...
if obj.tui:
    ...
    if choice != None:
        if choice == len(keys) - 1:
            ...
            sys.exit(0)
        printc("Tipo de extensiones seleccionadas: {}".format(key_capitalized[choice]), Colors.red_to_blue)
        ...
        if choice:
            ...
            if len(finded_file) < 1:
                ...
                print(finded_file)
        printc("Deseas comprimir los archivos encontrados?",Colors.red_to_blue)
        choice = termui.terminal(yes_no)
        if choice:
            ...
            comp_obj.add(files)
            comp_obj.finish()
            printc("Deseas subir el archivo comprimido a Google Drive?",Colors.red_to_blue)
            choice = termui.terminal(yes_no)
            if choice:
                obj_drive = Drive()
                ...
                print("Completado")
            printc("Deseas subir los archivos encontrados a Google Drive?", Colors.red_to_blue)
            choice = termui.terminal(yes_no)
            if choice:
                obj_drive = Drive()
                folder = obj_drive.create_folder("Subidas")
            ...
            printc("Deseas mover los archivos a otro lado?", Colors.red_to_blue)
            choice = termui.terminal(yes_no)
            if choice:
                files_handler.move()
    else:
